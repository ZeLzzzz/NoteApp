                ...
                <div class="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
                    <div>
                        ...
                        <div class="pb-20 flex justify-start text-start">
                            @auth
                                <label class="text-white lg:text-gray-400 text-lg md:text-xl">We sent you an email. Follow the instructions to change your account password.</label>
                            @else
                                <label class="text-white lg:text-gray-400 text-lg md:text-xl">We sent you an email. Follow the instructions to get back into your account.</label>
                            @endauth
                        </div>
                    </div>
                    <div class="pb-2 pt-4 text-white">
                        @auth
                            <a href="{{ route('home') }}" type="submit" class="uppercase block w-full p-3 md:p-4 text-xs md:text-lg rounded-full bg-blue-500 hover:bg-blue-600 focus:outline-none">Back to home</a>
                        @else
                            <a href="{{ route('login') }}" type="submit" class="uppercase block w-full p-3 md:p-4 text-xs md:text-lg rounded-full bg-blue-500 hover:bg-blue-600 focus:outline-none">Back to login</a>
                        @endauth
                    </div>
                </div>
            </div>
